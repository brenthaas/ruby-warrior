...
describe RubyWarrior::Game do
  ...
  it "should ask user to choose a tower when creating a new profile" do
    ...
    RubyWarrior::UI.expects(:choose).with('tower', [:tower1, :tower2]).returns(double(:path => '/foo/bar'))
    @game.new_profile
  end
  
  it "should pass name and selected tower to profile" do
    profile = double
    RubyWarrior::UI.stubs(:choose).returns(double(:path => 'tower_path'))
  ...
  it "should fetch current level from profile and cache it" do
    @game.stubs(:profile).returns(double)
